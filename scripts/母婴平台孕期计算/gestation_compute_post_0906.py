...
def repair_subject(platform_id, subject):
    '''
    修复subject
    :param platform_id:
    :param subject:
    ...
    '''
    if platform_id == '1001':
        new_subject = re.sub(" |首页>>|树>>|树下>>|下>>|首页>|树>|树下>|下>|首页|树|树下|下|←", "", subject)
        new_subject = re.sub(">>|&t;|&g;|gt;", ">", new_subject)
    elif platform_id == '1003':
        new_subject = re.sub(" ", "", subject)
        new_subject = re.sub("\\|", ">", new_subject)
    elif platform_id == '1004':
        new_subject = re.sub(" ", "", subject)
        new_subject = re.sub("\\|", ">", new_subject)
    elif platform_id == '1008':
        new_subject = re.sub(" ", "", subject)
        new_subject = re.sub("\\||>>", ">", new_subject)
    elif platform_id == '1005':
        new_subject = subject.split(">")[1]
    elif platform_id == '1006':
        new_subject = re.sub(" |辣妈帮首页>", "", subject)
        new_subject = new_subject.split(">")[0]
    elif platform_id == '1007':
        new_subject = re.sub("←", "", subject)
    elif platform_id == '1102':
        new_subject = re.sub(" ", "", subject)
        new_subject = re.sub("\\|", ">", new_subject)
    elif platform_id == '1103':
        new_subject = re.sub(" ", "", subject)
        new_subject = re.sub("\\|", ">", new_subject)
    elif platform_id == '1110':
        new_subject = re.sub(">帖子详情|>??帖子详情", "", subject)
    elif platform_id == '1016':
        new_subject = re.sub(" ", "", subject)
        new_subject = re.sub(">>", ">", new_subject)
    else:
        new_subject = subject

    if new_subject and platform_id in("1001","1003","1004","1005","1006","1007","1008","1009","1013","1015",
                      "1016","1101","1102","1103","1104","1105","1106","1107","1108","1109","1111","1112","1113","1114","1115"):
        if new_subject[-1] == ">":
            if new_subject[0] == " ":
                new_subject = new_subject[1: -1]
            else:
                new_subject = new_subject[0: -1]
        elif new_subject[0] == " ":
...
if __name__ == '__main__':
    for line in sys.stdin:
        try:
            ...
            subject = repair_subject(platform_id, subject)

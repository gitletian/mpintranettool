...
----------    rm -rf  ./*/*.zip
----------
----------    beeline -u jdbc:hive2://mphd02:10000 -n fengxiangqing --hivevar file_path='/home/data/motherbaby/20170829_1001/*.txt' --hivevar platform_id='1001' -f  /home/data/script/motherbaby_append.sql
----------
--------------------------------------------------------------------------------------
...
LOAD DATA LOCAL INPATH '${hivevar:file_path}'  INTO TABLE extract.motherbaby_post_txt_add;
...
drop table if exists transforms.motherbaby_post_simple_platform;
CREATE TABLE  if not exists transforms.motherbaby_post_simple_platform(
    id   string,
    channel   int,
    subject   string,
    post_id   string,
...
insert into transforms.motherbaby_post_simple_platform
SELECT
     id
    ,channel
    ,subject
    ...
    ,platform_id
FROM
transforms.motherbaby_post_add
where
nvl(error_info, '') = ''
;
...
l_motherbaby.post
where
platform_id = ${hivevar:platform_id}
;
...
drop table if exists transforms.motherbaby_post_repaire_floorid;
CREATE TABLE  if not exists transforms.motherbaby_post_repaire_floorid(
    id   string,
    channel   int,
    subject   string,
    post_id   string,
...
insert into transforms.motherbaby_post_repaire_floorid
SELECT
     id
    ,channel
    ,subject
    ...
    ,platform_id
FROM
transforms.motherbaby_post_simple_platform
where
is_host = 1
;
...
FROM
transforms.motherbaby_post_simple_platform t1 where t1.is_host != 1
) t2
...
select
    ...
    ,t1.platform_id
from
    (select * from transforms.motherbaby_post_simple_platform where user_id not in ('1005:#', '') ) t1
left join
...
SELECT
    ...
    ,s.platform_id
FROM
transforms.post_join_user AS s
LEFT JOIN
transforms.motherbaby_post_noise_add AS n
ON s.id = n.id
;
...
------------------合并---------------------
-- use l_motherbaby;
-- alter table l_motherbaby.post drop partition(platform_id='${hivevar:platform_id}');
--

...
----------    rm -rf  ./*/*.zip
----------    beeline -u jdbc:hive2://mphd02:10000 -n fengxiangqing --hivevar file_path='/home/data/medicine/20170829_1001/*.txt' --hivevar platform_id='1001' -f  /home/data/script/motherbaby_append.sql
----------
----------
--------------------------------------------------------------------------------------
...
LOAD DATA LOCAL INPATH '${hivevar:file_path}'  INTO TABLE extract.medicine_post_inquiry_txt_add;
...
drop table if exists transforms.medicine_post_inquiry_simple_platform;
CREATE TABLE  if not exists transforms.medicine_post_inquiry_simple_platform(
     id                 string
    ,channel            int
    ,subject            string
    ,post_id            string
...
insert into transforms.medicine_post_inquiry_simple_platform
SELECT
     id
    ,channel
    ,subject
    ...
    ,platform_id
FROM
transforms.medicine_post_inquiry_add_unique
;
...
select
    ...
    ,t2.platform_id
from
transforms.medicine_post_inquiry_simple_platform t2
where t2.is_host = 1
;
...
FROM
transforms.medicine_post_inquiry_simple_platform t1
where t1.is_host != 1
) t2
;
...
------------------------------------------------合并---------------------------------------------------
-- use l_medicine;
-- alter table l_medicine.post_inquiry drop partition(platform_id='${hivevar:platform_id}');

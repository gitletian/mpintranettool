...
drop table if exists mpintranet.similar_shop_shop;
CREATE TABLE  if not exists mpintranet.similar_shop_shop(
 shopid   STRING
,platformid bigint
,spu   STRING
,salesQty STRING
...
insert into table mpintranet.similar_shop_shop
select
t.shopid,
...
drop table if exists mpintranet.similar_shop_category;
CREATE TABLE  if not exists mpintranet.similar_shop_category(
 shopid   STRING
,Categoryname   STRING
,spu STRING
,SalesAmt STRING
...
insert into table mpintranet.similar_shop_category
...
-- 7、 各店铺各适用年龄段SPU数分布：店铺ID 适用年龄段1  适用年龄 段2 。。。 适用年龄段n      == 适用年龄

--- similar_shop_attr_age   适用年龄
--- similar_shop_attr_style  风格

drop table if exists mpintranet.similar_shop_attr_style;
CREATE TABLE  if not exists mpintranet.similar_shop_attr_style(
 shopid   STRING
,attrvalue   STRING
,spu STRING
)
...
insert into table mpintranet.similar_shop_attr_style
select
shopid,
concat_ws(",", collect_list(attrvalue)) as attrvalue,
concat_ws(",", collect_list(cast(spu as String))) as spu
from shopid_attr_join
group by shopid
;
...
drop table if exists mpintranet.similar_shop_data;
CREATE TABLE  if not exists mpintranet.similar_shop_data(
 shopid   STRING
...
with source_shop as (
    ...
    join
    mpintranet.similar_shop_attr_age age
    on shop.shopid = age.shopid
    join
...
target_shop as (
    ...
    join
    mpintranet.similar_shop_attr_age t_age
    on t_shop.shopid = t_age.shopid
    join
    mpintranet.similar_shop_attr_style t_style
    on t_shop.shopid = t_style.shopid
    where shopid = '67001977'
)


insert into table mpintranet.similar_shop_data
select
s.shopid,
...
add file /home/script/normal_servers/serverudf/elengjing/similar_shop.py;
...
select
  ...
  USING 'python similar_shop_8.py'
  AS (shopid, sim, error_info)
FROM
mpintranet.similar_shop_data
;

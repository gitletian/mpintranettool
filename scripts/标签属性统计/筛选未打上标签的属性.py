...
if __name__ == '__main__':
    for line in sys.stdin:
        try:
            ...
            desc_tag = data_array[3].split(",")
            untag_list = set(desc_tag).difference(set(data_array[4].split(",")))
            untag = ",".join(untag_list)
            export_data(untag, len(desc_tag), len(untag_list), '')

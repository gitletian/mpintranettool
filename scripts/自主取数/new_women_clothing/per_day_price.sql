--------------------------------------------------------------------------------------
----------                       初始化每一天的 自主计算数据
----------1、开始入库每一天的数据:  需要先执行 init_price.sql
----------2、运行beeline
---------- beeline -u jdbc:hive2://mphd02:10000 -n hive -p hive1  --hivevar CSV_ITEM_FILE=hy_item_2016_12_24.csv  --hivevar BEGIN_DAY=2016-12-22  --hivevar DATE_RANGE=2016-12-23 -f  ./per_day_price.sql
----------3、 此脚本使用于前30天的, 30天之后的需另作处理
--------------------------------------------------------------------------------------
...
-- LOAD DATA LOCAL INPATH '/home/data/tmp/item/data/work/${hivevar:CSV_ITEM_FILE}'  INTO TABLE mpintranet.women_clothing_item_load;

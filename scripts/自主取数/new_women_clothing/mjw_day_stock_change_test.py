...
ItemID, p_SKUList, y_SKUList, y_salesqty, m_DateRange, p_DateRange = list(np.random.choice([""], size=6))
...
def export_data(sku_day_salesqty, sku_day_salesamt, sku_day_stock_change, is_has_months_change, error_info):
    ...
    :return:
    '''
    print "\t".join([unicode(p_DateRange), unicode(ItemID), unicode(p_SKUList), unicode(sku_day_salesqty), unicode(sku_day_salesamt), unicode(sku_day_stock_change), unicode(is_has_months_change), unicode(y_salesqty), unicode(error_info)])
...
def sku_list_to_sku_map(skulist):
    ...
    for sku in skulist.split(":"):
        sku_array = sku.split(";")
        if len(sku_array) == 4:
            skuMap.update({sku_array[0]: dict(discount_price=sku_array[1], stock=sku_array[3])})
...
for line in sys.stdin:
    try:
        ItemID, p_SKUList, y_SKUList, y_salesqty, m_DateRange, p_DateRange = [_.replace("\\N", "") for _ in line.strip().split('\t')]
